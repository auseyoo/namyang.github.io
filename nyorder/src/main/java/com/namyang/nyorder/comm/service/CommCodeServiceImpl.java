...
public class CommCodeServiceImpl implements CommCodeService {
	 ...
	 */
	@Transactional
	public String saveCommCodeList(List<CommCodeVO> list) {
		for(CommCodeVO one : list) {
			System.out.println(one);
			if("C".equals(one.getCrudMode())) {
				commCodeMapper.insertCommCode(one);
			}else if("U".equals(one.getCrudMode())) {
				commCodeMapper.updateCommCode(one);
			}else if("D".equals(one.getCrudMode())) {
				commCodeMapper.delCommCode(one);
			}
		}
		return "저장되었습니다.";
	}
