...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#batchBtn").click(function(){
			...
			if( $("#batch").val() == "reset" ){
				...
				$.each(AUIGrid.getGridData(myGridID), function(k,v){
					if( v.wsQuantity01 ){
						AUIGrid.setCellValue(myGridID, k, "myQuantity01", v.wsQuantity01);
						if( emplCd == '10001' ){
							AUIGrid.updateRow(myGridID, {'storReqBoxQty': Number( v.wsQuantity01 )}, k);
						}else{
							AUIGrid.updateRow(myGridID, {'grfrReqBoxQty': Number( v.wsQuantity01 + grfrReqBoxQty )}, k);
						}
					}

					if( v.wsQuantity02 ){
						AUIGrid.setCellValue(myGridID, k, "myQuantity02", v.wsQuantity02);
						if( emplCd == '10001' ){
							AUIGrid.updateRow(myGridID, {'storReqIddyQty': Number( v.wsQuantity02 )}, k);
						}else{
							AUIGrid.updateRow(myGridID, {"grfrReqIddyQty": Number(v.wsQuantity02 + grfrReqIddyQty)} , k);
						}
					}
				});
			}
	...
	var columnLayout = [{
			...
			dataField : "wsQuantity",
			headerText : "예상수량",
				children : [{
					dataField : "wsQuantity01",
					headerText : "BOX",
					width : "3.5%",
					style: "auiRight",
					editable : false
				}, {
					dataField : "wsQuantity02",
					headerText : "낱봉",
					width : "3.5%",
					style: "auiRight",
					editable : false
