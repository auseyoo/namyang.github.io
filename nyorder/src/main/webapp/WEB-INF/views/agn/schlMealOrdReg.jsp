<!--
	@File Name: schlMealOrdReg
	@File 설명 : 학교급식 주문 등록
	@UI ID : 	UI-PORD-0401.html
	@작성일 : 2022.02.09
	@작성자 : 윤이준
 -->
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#batchBtn").click(function(){
			...
			if( $("#batch").val() == "reset" ){
				...
				$.each(AUIGrid.getGridData(myGridID), function(k,v){
					if( v.eveReqBoxQty ){
						AUIGrid.setCellValue(myGridID, k, "myQuantity01", v.eveReqBoxQty);
						if( emplCd == '10001' ){
							AUIGrid.updateRow(myGridID, {'storReqBoxQty': Number( v.eveReqBoxQty )}, k);
						}else{
							AUIGrid.updateRow(myGridID, {'grfrReqBoxQty': Number( v.eveReqBoxQty + grfrReqBoxQty )}, k);
						}
					}

					if( v.eveReqIddyQty ){
						AUIGrid.setCellValue(myGridID, k, "myQuantity02", v.eveReqIddyQty);
						if( emplCd == '10001' ){
							AUIGrid.updateRow(myGridID, {'storReqIddyQty': Number( v.eveReqIddyQty )}, k);
						}else{
							AUIGrid.updateRow(myGridID, {"grfrReqIddyQty": Number(v.eveReqIddyQty + grfrReqIddyQty)} , k);
						}
					}
				});
			}
	...
	var columnLayout = [{
			...
			headerText : "직전주문",
				children : [{
					dataField : "eveReqBoxQty",
					headerText : "BOX",
					width : "4%",
					style: "auiRight"
				}, {
					dataField : "eveReqIddyQty",
					headerText : "낱봉",
					width : "4%",
					style: "auiRight"
				}]
	...
	/* 표준 제품 - 리스트 조회 */
	function searchList(){
		var item = { "ordDt" : $("#ordDt").val() , "vendSeq" : $("#vendSeq").val() , "ordType" : $("#ordType").val() };
		
		$.ajax({
			url : "/agn/selectSchlMealPrdList.do",
			type : 'POST',
	...
	function selectGrfrOrdCnt(){
		var item = { "ordDt" : $("#ordDt").val() , "ordType" : $("#ordType").val() , "vendSeq" : $("#vendSeq").val() };
		$.ajax({
...
<div class="rContentBox">
	<div class="content">
		...
		<form id="frm">
			...
			<input type="hidden" name="ordType" id="ordType" value="SCHL">
			...
			<div class="inquiryBox">
				<dl>
					...
					<dd>
						<div class="formWrap">
							<div class="dateWrap">
								<input type="text" value="" class="inp" id="datepicker01">
				...
				<div class="titleArea right">
					...
					<div class="formGroup">
						...
						<select id="batch" class="sel small w120">
							...
							<option value="preOrd">직전주문</option>
						</select>
