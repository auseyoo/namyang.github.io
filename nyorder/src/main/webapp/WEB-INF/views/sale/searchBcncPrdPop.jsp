...
$(document).ready(function(){
	...
	AUIGrid.bind(popGrid, "selectionChange", function(event) {
		...
		if(confirm("해당제품을 선택하시겠습니까?")){
			if(targetNotGrid == true){
				$("#sprtPrdDtlSeq").val(rowItem.prdDtlSeq);
				$("#sprtPrdCd").val(rowItem.prdDtlCd);
				$("#sprtPrdNm").val(rowItem.prdNm);
			}else if(prdBndChg ==true){
				selectAllBcncUntpc(rowItem);
			}else{
				if(untpcPage){
					var returnYn =false;
					AUIGrid.getGridData(mGrid2).forEach(function(data){
						if(data.prdDtlSeq ==rowItem.prdDtlSeq){
							returnYn= true;
							return;
						}
					});
				...
				AUIGrid.updateRow(
				mGrid2
				,{  'prdDtlSeq':rowItem.prdDtlSeq,
					'puchSeq':rowItem.puchSeq,
					'prdDtlSeq':rowItem.prdDtlSeq,
					'prdDtlCd':rowItem.prdDtlCd ,
					'prdNm':rowItem.prdNm,
					'faltQty':rowItem.faltQty,
					'iddyPchaCt':rowItem.iddyPchaCt,
					'saleUntpc':rowItem.saleUntpc
				}, 0);
			}
...
function searchPrdList(){
	...
	if(targetNotGrid == true){
	 ...
	 $.ajax({
        url: "/sale/selectStdPrdPopList.do",
        type: 'POST',
        data:{
        	  "schType" :$("#schType").val(),
        	  "schTxt" :$("#schTxt").val(),
...
<div class="popArea">
	<%-- 제품 등록 레이어 S --%>
	<div class="popWrap mideum modal_wrap" data-popup="srcBcncPrdReg" style="left:0; right:0;">
		<header>
			<h3>제품 등록</h3>
		...
		<div class="popCon">
			...
			<div class="inquiryBox dtShort">
				<dl>
					...
					<dd>
						<div class="formWrap">
							<select name="" class="sel w120 mr10" id="schType">
								<option value="">전체</option>
								<option value="prdNm">제품명</option>
								<option value="prdSapCd">제품코드</option>
							</select>
						</div>
					</dd>
					...
					<dd>
						<div class="formWrap">
							<input type="text" class="inp w160 mr7" value="" id="schTxt">
							<button type="button" name="" class="comBtn" id="layerSearchBtn" onclick = 'searchPrdList()'>조회</button>
						</div>
					</dd>
				</dl>
			</div>
