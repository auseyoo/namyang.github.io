USE [NY_ORDER]
GO
/****** Object:  UserDefinedFunction [dbo].[F_GET_UNTPC]    Script Date: 2022-03-15 오후 12:58:48 ******/
SET ANSI_NULLS ON
GO
...
-- Description:	판매가
-- =============================================
CREATE FUNCTION [dbo].[F_GET_UNTPC]
(
...
BEGIN
	...
	--@V_CST_PRD_U_CNT
		...
		END
	END
	IF @O_UNTPC >0 BEGIN RETURN @O_UNTPC END
	...
	BEGIN
		SELECT
			@O_UNTPC = (CASE
				WHEN UNTPC IS NULL THEN ISNULL(SPPRC, 0) + ISNULL(VAT_SPPRC,0)
				WHEN UNTPC IS NULL AND SPPRC IS NULL AND VAT_SPPRC IS NULL THEN 0
			ELSE UNTPC END )
		FROM
			T_PRD_MST
		WHERE
